...
<div class="d-flex justify-content-between align-items-center">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Add Employe</h6>
        </div>
        <div class="card-body">
            {{-- HTML outiside the template start --}}
            <form action="{{ isset($employee) ? route('admin.employees.update', $employee->user_id) : route('admin.employees.store') }}" method="POST">
                @csrf
                @if(isset($employee))
                @method('PUT')
                @endif
                ...
                <div class="row">
                    ...
                    <div class="mb-3 col-md-6">
                        <label>Phone:</label>
                        <input type="text" name="mobile_phone" value="{{ old('mobile_phone', $employee->mobile_phone ?? '') }}" class="form-control">
                        @error('mobile_phone')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
